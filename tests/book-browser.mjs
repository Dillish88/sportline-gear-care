...
await db.exec(fs.readFileSync('db/pilot-booking-v3.sql','utf8'));await db.exec(fs.readFileSync('db/pilot-loyalty-baseline.sql','utf8'));await db.exec(fs.readFileSync('db/pilot-loyalty-hardening.sql','utf8'));
...
try{
 ...
 await customer.locator('#cbOpen').click();await customer.getByRole('button',{name:'Help me pick a string',exact:true}).click();await customer.getByRole('button',{name:'Just started',exact:true}).click();await customer.getByRole('button',{name:'1–2 times a week',exact:true}).click();await customer.getByRole('button',{name:'Feather',exact:true}).click();await customer.getByRole('button',{name:'A bit of both',exact:true}).click();await customer.locator('#cbFrame').fill('20');await customer.getByRole('button',{name:'Check suggestion',exact:true}).click();await customer.getByRole('button',{name:'Use this in my booking',exact:true}).click();assert(await customer.locator('#flow').isVisible());assert.equal(await customer.locator('#str').inputValue(),'Yonex|BG65');assert.match(await customer.locator('#crossesOut').innerText(),/^20/);await customer.locator('#exactToggle').click();await customer.locator('#days input').first().waitFor();await customer.locator('#days input').nth(1).check();await customer.locator('#slots input:not([disabled])').first().check();
 ...
 assert(await customer.evaluate(()=>{const a=document.getElementById('cbOpen').getBoundingClientRect(),b=document.getElementById('submit').getBoundingClientRect();return a.bottom<=b.top||a.right<=b.left||a.left>=b.right;}));await customer.screenshot({path:path.resolve('../archive-review/approved-booking-form.png'),fullPage:true});
 ...
 staff.on('dialog',d=>d.accept());await staff.evaluate(()=>window.print=()=>{});await staff.getByText('WhatsApp, print & other actions',{exact:true}).click();await staff.getByRole('button',{name:'Print slip',exact:true}).click();assert.equal(await staff.locator('#slip img').count(),0);assert.match(await staff.locator('#slip').textContent(),/<img src=x/);
 ...
 await staff.getByText('WhatsApp, print & other actions',{exact:true}).click();const updateLink=new URL(await staff.getByRole('link',{name:'WhatsApp update',exact:true}).getAttribute('href'));assert.match(updateLink.searchParams.get('text'),new RegExp(code));assert.match(updateLink.searchParams.get('text'),/Status: https?:.*status.html#t=/);
