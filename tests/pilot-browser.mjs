...
try{
 ...
 await customer.locator('#brand-chips input').first().waitFor();
 await customer.locator('#colour-chips input[value="Lavender"]').check();
 assert.equal(await customer.locator('#colour').inputValue(),'Lavender');
 assert.equal(await customer.locator('#mobile-total').innerText(),'₹550');
 await customer.evaluate(()=>document.fonts.ready);
 await customer.screenshot({path:path.resolve('../archive-review/pilot-customer-refresh.png'),fullPage:true});
 ...
 assert.equal(await staff.locator('.job-card .primary').count(),1);
 await staff.locator('[data-view="Ready"]').click();await staff.getByText('No jobs in this view.').waitFor();
 await staff.locator('[data-view="Requested"]').click();await staff.locator('.job-card').waitFor();
 await staff.evaluate(()=>document.fonts.ready);
 await staff.screenshot({path:path.resolve('../archive-review/pilot-staff-refresh.png'),fullPage:true});
 await staff.setViewportSize({width:360,height:800});
 await staff.screenshot({path:path.resolve('../archive-review/pilot-staff-mobile.png'),fullPage:true});
 assert(await staff.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
 await staff.locator('.whatsapp-details summary').click();
 ...
 await staff.locator('.whatsapp-details summary').click();
 await staff.locator('[data-view="active"]').click();
 await staff.locator('.job-card input[type=number]').fill('550');await staff.getByRole('button',{name:'Accept job',exact:true}).click();
 await staff.locator('[data-view="working"]').click();await staff.getByRole('button',{name:'Start work',exact:true}).waitFor();
 ...
 await staff.locator('[data-view="Ready"]').click();await staff.getByRole('button',{name:'Record payment',exact:true}).waitFor();
 ...
 assert.equal(await customer.locator('#progress [aria-current="step"]').innerText(),'Ready');
